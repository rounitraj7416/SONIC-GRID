...
function renderResults(results) {
  ...
  container.innerHTML = `
    ...
    ${results.map((n, i) => `
      <div class="result-card" style="animation-delay:${i * 0.08}s">
        ...
        <div class="rc-chart-container" style="height: 60px; margin: 16px 0; position: relative;" title="24h Noise Trend (Shaded area = Sleep Window 10PM-7AM)">
          <canvas class="rc-noise-chart" id="chart-${i}"></canvas>
        </div>
  ...
  results.forEach((n, i) => {
    ...
    // Generate mock 24h noise data based on avgDb
    const baseDb = n.avgDb;
    const hours = Array.from({length: 24}, (_, idx) => idx);
    const data = hours.map(h => {
      let db = baseDb;
      if (h >= 8 && h <= 11) db += (Math.random() * 5 + 5); // morning rush
      else if (h >= 17 && h <= 20) db += (Math.random() * 5 + 7); // evening rush
      else if (h >= 22 || h <= 6) db -= (Math.random() * 5 + 5); // night
      else db += (Math.random() * 4 - 2); // day normal
    ...
    const chart = new Chart(canvas, {
      ...
      data: {
        ...
        datasets: [{
          data: data,
          borderColor: getQuietColor(n.quietScore),
          borderWidth: 2,
          pointRadius: 0,
          pointHoverRadius: 4,
          tension: 0.4
      ...
      plugins: [{
        id: 'sleepWindowBackground',
        beforeDraw: (chart) => {
          const { ctx, chartArea, scales: { x } } = chart;
          if (!chartArea) return;
          const { top, bottom, right } = chartArea;
          ctx.save();
          ctx.fillStyle = 'rgba(139, 92, 246, 0.15)'; // faint violet shading for sleep window
          
          const x0 = x.getPixelForValue(0);
          const x7 = x.getPixelForValue(7);
          const x22 = x.getPixelForValue(22);
          
          if (x7 > x0) ctx.fillRect(x0, top, x7 - x0, bottom - top);
          if (right > x22) ctx.fillRect(x22, top, right - x22, bottom - top);
...
export function initPlanner() {
  ...
  // Preload UI with default suggestions
  const initialResults = [...NEIGHBORHOODS].sort((a, b) => a.avgDb - b.avgDb).slice(0, 3).map(n => ({ ...n, finalScore: n.quietScore }));
  renderResults(initialResults);
  const header = document.querySelector('.results-header');
  if (header) {
    header.innerHTML = '<span class="results-count">🌟 Top 3 Quietest Neighborhoods</span><span class="results-sort">Default Preview</span>';
  }
  ...
  if (section) {
    const obs = new IntersectionObserver((entries) => {
      entries.forEach(e => {
        if (e.isIntersecting) {
          initPlannerMap();
          renderMapMarkers(initialResults, 'any');
          obs.disconnect();
        }
      });
    }, { threshold: 0.1 });
